...
class DT_Classifier:
    ...
    def fit(self, X, y, sample_weighted=None):
        """
    ...
    def node_weighted_sum(self, node):
        weighted_sum = 0
        for idx in node.data_idx:
            weighted_sum += self.sample_weighted[idx]
        return weighted_sum
    ...
    def choose_feature_and_branch(self, node):
        ...
        for feature_idx in range(self.feature_size):
            ...
            for val in split_threshold_candidate:
                ...
                information_gain = self.node_weighted_sum(node) * -self.compute_entropy(
                    node) + self.node_weighted_sum(left_child) * self.compute_entropy(
                        left_child) + self.node_weighted_sum(right_child) * self.compute_entropy(right_child)
